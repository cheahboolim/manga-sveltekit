			...
			{#if isMobile}
				...
				<div class="flex w-full items-center justify-between gap-3">
					...
					<div class="flex-shrink-0">
						<a href="/" class="font-bold text-lg sm:text-xl text-foreground">
							<span class="block sm:hidden">SM</span>
							<span class="hidden sm:block">SUSMANGA</span>
						</a>
					</div>

					<!-- Mobile Search -->
					<div class="flex-1 max-w-sm mx-3">
						<form on:submit={handleSearch} class="relative">
							<input
								type="search"
								bind:value={search}
								placeholder="Search..."
								class="w-full rounded-full text-sm placeholder:text-gray-400 bg-[#343434] text-white px-4 py-2 pr-10 border border-white/20 focus:border-white/40 focus:outline-none transition-colors"
							/>
							<button
								type="submit"
								class="absolute right-1 top-1/2 transform -translate-y-1/2 bg-transparent hover:bg-white/10 text-white p-1.5 rounded-full transition-colors"
								aria-label="Search"
							>
					...
					<div class="flex-shrink-0">
						<button 
							type="button" 
							class="text-foreground hover:text-foreground/80 p-2 -m-2 transition-colors" 
...
{#if isMobile && $mobileMenuOpen}
	...
	<!-- Mobile Menu Panel -->
	<div class="fixed top-0 right-0 bottom-0 w-[85%] max-w-sm z-[101] bg-black/95 backdrop-blur-md border-l border-white/10">
		<div class="flex flex-col h-full">
			...
			<div class="flex-1 overflow-y-auto p-4">
				<!-- Random Button -->
				<div class="mb-6">
					<a href="/p/random" class="block" on:click={closeMobileMenu}>
						<div class="bg-[#FF1493] hover:bg-[#e01382] text-white px-6 py-3 rounded-lg text-center transition-colors text-base font-medium">
							Random
						</div>
					</a>
				</div>
				...
				<nav class="space-y-1">
					{#each navItems.slice(1) as item}
						<a
							href={item.href}
							class="block text-white/90 hover:text-white hover:bg-white/5 transition-colors rounded-lg {get(page).url.pathname === item.href ? 'bg-white/10 text-white' : ''}"
							on:click={closeMobileMenu}
						>
